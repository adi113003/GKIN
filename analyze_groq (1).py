"""
analyze_groq.py — Runs Llama 3.3 70B via Groq over every article in the
labeling sheet and writes structured analysis results back.

Use this when Gemini quota is unavailable. Output schema is identical to
the Gemini version, so downstream code doesn't need to change.

Setup:
  1. Get a free API key at https://console.groq.com/keys
  2. export GROQ_API_KEY="your_key_here"
  3. pip install groq openpyxl
  4. python analyze_groq.py GKIN_Eval_Labeling.xlsx

Outputs:
  - New columns written to the articles sheet:
      groq_manipulation_index, groq_narrative_cluster,
      groq_persuasion_count, groq_claims_count
  - Full structured output saved to: groq_results.json
...
def main(xlsx_path: str):
    ...
    cols = ensure_columns(ws, [
        "groq_manipulation_index",
        "groq_narrative_cluster",
        "groq_persuasion_count",
        "groq_claims_count",
    ])
    col_id = cols.get("id")
    col_text = cols["full_text"]
    col_mi = cols["groq_manipulation_index"]
    col_nc = cols["groq_narrative_cluster"]
    col_pc = cols["groq_persuasion_count"]
    col_cc = cols["groq_claims_count"]
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python analyze_groq.py <path_to_xlsx>")
        sys.exit(1)
    main(sys.argv[1])
